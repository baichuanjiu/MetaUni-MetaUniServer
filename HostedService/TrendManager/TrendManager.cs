...
namespace HostedService.TrendManager
{
    public class TrendManagerHostService : BackgroundService
    {
        ...
        private async void InitTrendList()
        {
            ...
            if (miniAppRedis.KeyExists($"TrendList{GetCycleSuffix(now)}"))
            {
                await miniAppRedis.SortedSetCombineAndStoreAsync(SetOperation.Union, $"TrendList{GetCycleSuffix(next)}", keys: new RedisKey[] { $"TrendList{GetCycleSuffix(now)}", $"TrendCycle{GetCycleSuffix(expired)}" }, weights: new double[] { 1, -1 }, aggregate: Aggregate.Sum);
            ...
            if (feedRedis.KeyExists($"TrendList{GetCycleSuffix(now)}"))
            {
                await feedRedis.SortedSetCombineAndStoreAsync(SetOperation.Union, $"TrendList{GetCycleSuffix(next)}", keys: new RedisKey[] { $"TrendList{GetCycleSuffix(now)}", $"TrendCycle{GetCycleSuffix(expired)}" }, weights: new double[] { 1, -1 }, aggregate: Aggregate.Sum);
